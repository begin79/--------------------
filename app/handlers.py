...
async def handle_mode_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if default_query is None:
        ...
        prompt = (
            ...
            f"Теперь вы можете искать расписание для любой группы/преподавателя.\n"
            f"Просто введите название группы или ФИО преподавателя.\n\n"
            f"💡 Чтобы установить группу/преподавателя по умолчанию для уведомлений, "
        ...
        # Для существующих пользователей показываем стандартный запрос (для изменения группы)
        prompt = "🎓 Введите название группы:" if mode == MODE_STUDENT else "🧑‍🏫 Введите ФИО преподавателя:"
